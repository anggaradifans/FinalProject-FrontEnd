...
class Wishlist extends React.Component {
  ...
  getDataWishlist = () => {
    Axios.get(urlApi + "/product/wishlist/" + this.props.id).then(res => {
  ...
  renderJsx = () => {
    var jsx = this.state.data.map(val => {
      return (
        ...
        >
          <Link to={"/product-detail/" + val.idproduk}>
            <img
          ...
          <div className="card-body">
            ...
            <input
              ...
              value="Add to Cart"
              onClick={() => this.onBtnAddtoCart(val.idproduk)}
            />
          </div>
        </div>
      );
  ...
  render() {
    return (
      <div className="container">
        ...
        <div className="row">{this.renderJsx()}</div>
      </div>
    );
  }
}
