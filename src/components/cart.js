...
class CustomPaginationActionsTable extends React.Component {
  ...
  onBtnSave = () => {
    var quantity =
      this.quantityEdit.inputRef.value === ""
  ...
  getTotalHarga = () => {
    ...
    for (var i = 0; i < this.state.rows.length; i++) {
      harga += parseInt(
        (this.state.rows[i].price -
          (this.state.rows[i].price * this.state.rows[i].discount) / 100) *
  ...
  deleteCart = () => {
    for (var i = 0; i < this.state.rows.length; i++) {
      Axios.delete(urlApi + "/cart/deletecart/" + this.state.rows[i].id).then(
        res => {
          this.props.fnHitungCart(this.props.username);
          this.getDataApi();
        }
  ...
  checkOut = () => {
    ...
    Axios.post(urlApi + "/cart/checkout", newData).then(res => {
      this.addToTransactionDetail();
      this.deleteCart();
      swal(
        "Success",
        "Invoice Sent to Email, Please Upload Your Receipt",
        "success"
      );
          ...
          <TableRow key={val.id}>
            ...
            <TableCell>
              <Button
                animated
                color="teal"
                onClick={() => this.onBtnEditClick(val)}
              >
                ...
                <Table>
                  <TableHead>
                    <TableRow>
                      ...
                      >
                        EDIT QUANTITY PRODUCT {namaProduk}
                      </TableCell>
                    </TableRow>
                  </TableHead>
                  <TableBody>
                    <TableRow>
                      <TableCell>
                        <Input
                          ref={input => (this.quantityEdit = input)}
                ...
                <Table className={classes.table}>
                  ...
                  <TableFooter>
                    <TableRow>
                      <TableCell colSpan={4}>
                        <h3>
                          Total Prices : {formatMoney(this.getTotalHarga())}
                        </h3>
                      </TableCell>
                      <TableCell colSpan={1}>
                        <Link to="/products">
...
export default connect(mapStateToProps, { fnHitungCart, resetCount })(
