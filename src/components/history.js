...
class History extends React.Component {
  ...
  getDataUrl = () => {
    ...
    if (this.props.location.search) {
      if (obj.month < 10) {
        Axios.get(
          urlApi +
            `/trans/filterhistory?iduser=${this.props.id}&month=0${obj.month}`
        )
          .then(res => {
            this.setState({ rows: res.data });
          })
        ...
        Axios.get(
          urlApi +
            `/trans/filterhistory?iduser=${this.props.id}&month=${obj.month}`
        )
          .then(res => {
            this.setState({ rows: res.data });
          })
    ...
    var jsx = this.state.rows.map(val => {
      return (
        <tr>
          ...
          <Link to={"/history-detail/" + val.order_number}>
  ...
  Dropdown = () => {
    return (
      <select ref="bulan" className="form-control">
  ...
  filterData = () => {
    ...
    if (parseInt(bulan) === 0) {
      ...
      Axios.get(
        urlApi + `/trans/filterhistory?iduser=${this.props.id}&month=${bulan}`
      )
        .then(res => {
          this.setState({ rows: res.data });
        })
      ...
      return (
        <div className="container">
          <h2 style={{ marginTop: "5px" }}>
            Transactions History for {this.props.username}
          </h2>
          <h4>Filter by Month</h4>
          <div className="row">
            <div className="col-md-3">{this.Dropdown()}</div>
