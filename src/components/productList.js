...
class ProductList extends React.Component {
  ...
  getDataProduct = async () => {
    try {
      let product = await axios.get(urlApi + "/product/products");
      if (!product) {
  ...
  getSubcategory = () => {
    axios
      .get(urlApi + "/category/subcategory")
  ...
  onBtnAddtoCart = id => {
    ...
    axios
      .post(urlApi + "/cart/addtocart", newData)
      .then(res => {
  ...
  renderProdukJsx = () => {
    var arrSearchAndFilter = this.state.listProduct.filter(val => {
      return (
        ...
        (val.category === this.state.filterCategory ||
          ...
          <div className="card-body">
            ...
            {this.props.username === "" ? (
              ...
              <input
                ...
                className="btn btn-primary"
                value="Add to Cart"
                onClick={() => this.onBtnAddtoCart(val.id)}
              />
            )}
          </div>
        </div>
  ...
  render() {
    ...
    return (
      <div className="container">
          ...
          <div className="col-md-2">
            <select
              ...
              onChange={() => {
                this.pushUrl();
                this.setState({
                  filterCategory: this.refs.dropdown.value,
                  dataPerpage: 6
                });
              }}
              className="form-control"
          ...
          <div className="col-md-2">
            <select
              ...
              onChange={() => {
                this.pushUrl();
                this.setState({
                  filterSub: this.refs.subcat.value,
                  dataPerpage: 6
                });
              }}
              className="form-control"
        ...
        <div className="row justify-content-center">
          {this.state.dataPerpage > this.state.listProduct.length ||
          arrSearchAndFilter.length < this.state.dataPerpage + 1 ? null : (
            <p
