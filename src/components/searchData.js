...
class Search extends React.Component {
  ...
  componentWillReceiveProps(newProps) {
    if (newProps.search) {
      ...
      this.props.getSearchData(newProps.search);
      this.getDataProduct();
    }
  }
  ...
  getDataProduct = () => {
    axios
      .get(urlApi + `/product/getsearchdata?pn=${this.props.search}`)
      ...
      return (
        ...
        >
          ...
          <div className="card-body">
            ...
            <CurrencyFormat
              value={val.price - val.price * (val.discount / 100)}
              displayType={"text"}
              thousandSeparator={true}
              prefix={"Rp"}
              renderText={value => (
      ...
      <div className="container">
        <div className="row justify-content-center">
          <h3 style={{ margin: "10px" }}>
            Search Result(s) for '{this.props.search}' :{" "}
            {!this.props.search ? 0 : this.state.listProduct.length}
          </h3>
        </div>
...
export default connect(mapStateToProps, { fnHitungCart, getSearchData })(
  Search
);
